...
def fno_predict_single_step(teacher_model, xb):
    ...
    N = 2000  # ← 생성 데이터셋의 N (미리 고정 또는 xb에서 알 수 있으면 동적으로)
...
def transfer_train(target_model, source_model, 
    ...
    distill_dataset = FNOSingleStepDistillDataset(file_list, source_model, device=device)
...
def run_fnotopinn_pipeline():
    ...
    transfer_train(pinn_student, fno_teacher, data_dir='data/output', num_epochs=500, model_save_path="fno_to_pinn.pth", log_path='fno_to_pinn.log')
    transfer_train(fpinn_student, fno_teacher, data_dir='data/output', num_epochs=500, model_save_path="fno_to_fpinn.pth", log_path='fno_to_fpinn.log')
    transfer_train(respinn_student, fno_teacher, data_dir='data/output', num_epochs=500, model_save_path="fno_to_respinn.pth", log_path='fno_to_respinn.log')
    
if __name__ == "__main__":
    run_fnotopinn_pipeline()
    print("FNO to PINN transfer learning completed.")