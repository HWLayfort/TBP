...
def run_multitask_fpinn_pipeline(train_loader, val_loader, test_loader, x_scaler, y_scaler, device='cuda'):
    ...
    train_multitask_fpinn(
        ...
        model_save_path=os.path.join(os.path.dirname(__file__), 'models', 'fno_to_fpinn.pth'),
        log_path=os.path.join(os.path.dirname(__file__), 'logs', 'fno_to_fpinn.log'),
        lambda_data=1.0,
        lambda_phys=1e-5
    )
    ...
    train_multitask_fpinn(
        ...
        num_epochs=500, lr=1e-3, device=device,
        x_scaler=x_scaler, y_scaler=y_scaler,
        model_save_path=os.path.join(os.path.dirname(__file__), 'models', 'tfno_to_fpinn.pth'),
        log_path=os.path.join(os.path.dirname(__file__), 'logs', 'tfno_to_fpinn.log'),
        lambda_data=1.0,
        lambda_phys=1e-5
    )
    test_multitask_fpinn(
    ...
    train_multitask_fpinn(
        ...
        num_epochs=500, lr=1e-3, device=device,
        x_scaler=x_scaler, y_scaler=y_scaler,
        model_save_path=os.path.join(os.path.dirname(__file__), 'models', 'deeponet_to_fpinn.pth'),
        log_path=os.path.join(os.path.dirname(__file__), 'logs', 'deeponet_to_fpinn.log'),
        lambda_data=1.0,
        lambda_phys=1e-5,
