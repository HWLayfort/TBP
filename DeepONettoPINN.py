...
class DeepONetSingleStepDistillDataset(torch.utils.data.Dataset):
    def __init__(self, filelist, teacher_model, device='cpu'):
        ...
        for fname in filelist:
            ...
            trunk_in = t_arr[:, None]   # (N, 1)

            # DeepONet trajectory inference
            branch_tensor = torch.tensor(branch_in[None, :], dtype=torch.float32, device=device)  # (1, 21)
            trunk_tensor = torch.tensor(trunk_in[None, ...], dtype=torch.float32, device=device)   # (1, N, 1)
            with torch.no_grad():
                y_traj_pred = teacher_model(branch_tensor, trunk_tensor)  # (1, 9, N)
...
def deeponet_predict_single_step(teacher_model, xb, t_min=0, t_max=10, N=2000):
    """
    xb: (batch, 22)
    - [0]: t
    - [1:10]: r0
    ...
    m  = xb[:, 19:22]
    batch = xb.shape[0]
    t_all = np.linspace(t_min, t_max, N, dtype=np.float32)  # (N,)
    branch = torch.cat([r0, v0, m], dim=1)                  # (batch, 21)
    trunk = torch.tensor(t_all, dtype=torch.float32, device=xb.device).unsqueeze(1).expand(N, 1) # (N, 1)
    trunk = trunk.unsqueeze(0).repeat(batch, 1, 1)  # (batch, N, 1)

    with torch.no_grad():
        y_traj = teacher_model(branch, trunk)    # (batch, 9, N)
    idxs = [np.abs(t_all - float(tv.cpu())).argmin() for tv in t_val]
    y_teacher = torch.stack([y_traj[i, :, idxs[i]] for i in range(batch)], dim=0)  # (batch, 9)
    return y_teacher

def train_single_step_distill_deeponet(
    teacher_model, student_model, 
    train_loader, val_loader, test_loader, 
    num_epochs=100, lr=1e-3, device='cuda',
    lambda_phys=1.0, lambda_distill=1.0,
    model_save_path='deeponet_to_pinn.pth', 
    log_path=None,
    ...
    for epoch in range(num_epochs):
        ...
        for xb, yb, mb in tqdm(train_loader, desc=f"Train {epoch+1}", leave=False):
            ...
            # distillation loss
            with torch.no_grad():
                y_teacher = deeponet_predict_single_step(teacher_model, xb, N=N_traj)
        ...
        val_loss, val_distill, val_phys = eval_single_step_distill_deeponet(
            teacher_model, student_model, val_loader, device, loss_fn, lambda_phys, lambda_distill, N_traj=N_traj
        )
...
def eval_single_step_distill_deeponet(
    teacher_model, student_model, loader, device, loss_fn, lambda_phys, lambda_distill, N_traj=2000
):
    teacher_model.eval()
    student_model.eval()
    total_loss, total_distill, total_phys, n = 0.0, 0.0, 0.0, 0
...
def transfer_train(
    target_model, deeponet_teacher, data_dir=None, model_save_path='deeponet_to_pinn.pth', 
    log_path=None, num_epochs=100, batch_size=1024, lambda_phys=1.0, lambda_distill=1.0, 
    early_stop_patience=20, device=None, N_traj=2000):
    if device is None:
        device = 'cuda' if torch.cuda.is_available() else 'cpu'
    deeponet_teacher = deeponet_teacher.to(device)
    target_model = target_model.to(device)
    if data_dir is None:
        data_dir = os.path.join(os.path.dirname(__file__), "data", "output")
    file_list = [os.path.join(data_dir, fname) for fname in os.listdir(data_dir) if fname.endswith('.csv')]

    # DeepONet Trajectory → 단일 시점 distillation dataset 생성
    distill_dataset = DeepONetSingleStepDistillDataset(file_list, deeponet_teacher, device=device)
    ...
    train_single_step_distill_deeponet(
        deeponet_teacher, target_model,
        train_loader, val_loader, test_loader,
...
def run_deeponettopinn_pipeline():
...
if __name__ == "__main__":
    ...
    print("DeepONet to PINN transfer learning pipeline completed.")